  ...
  <div class="container-fluid col-12 m-auto pt-5">
    ...
    <nav aria-label="breadcrumb" class="row card mx-5 px-3 pt-2 border-dark text-dark">
      <ol class="breadcrumb card-body">
        ...
        <li class="breadcrumb-item active" aria-current="page">{{ getDepartmentName() }}</li>
      ...
      <table id="example" class="card-body px-0 table" style="width: 100%;">
        <thead>
          <tr>
            ...
            <th scope="col" style="width: 30%;">Course Code</th>
          </tr>
        </thead>
        <tbody>
          <router-link v-for="(course, index) in courses" :key="course.id" :to="{ name: 'course-options', params: {courseId: course.id} }" custom v-slot="{ navigate }">
            <tr @click="navigate">
...
export default {
  ...
  computed: {
    courses() {
      return this.$store.getters.getCoursesByDeptId(this.$route.params.deptId)
    }
  },
  methods: {
    getUniversityName() {
  ...
  created() {
    ...
    this.facultyId = this.$store.getters.getDepartmentDataById('facultyId', parseInt(this.$route.params.deptId));
